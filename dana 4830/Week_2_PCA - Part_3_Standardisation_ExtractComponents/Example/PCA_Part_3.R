...
PCAscores <- data.frame(res.pca$scores)
sd(PCAscores$Comp.2)

mean(PCAscores$Comp.2)

Sscore <- data.frame(scale(PCAscores,center = T, scale=T))
sd(Sscore$Comp.1)



varimax7 <- varimax(res.pca$loadings)
varimax7



library(psych)
fit <- principal(Spcdf, nfactors=4, rotate="varimax")
