...
#### b. Use the method of least squares to estimate the unknown parameters 𝛽0, 𝛽1, and 𝛽2 of the model.
...
ME
```
Thus, we expect the model to provide predictions of prices to within about 2s= 2*(133.48) = $267 . Or, about 95% of sample prices fall within +-$267  of their predicted values using the model.
...
```
#### g. Form a 95% confidence interval for 𝛽1 and interpret the result.
...
The collector's hypothesis suggests that the effect of age on auction price depends on the number of bidders. This means there is a potential interaction effect between AGE and NUMBIDS.
...
```
The significant interaction term (AGE:NUMBIDS) confirms the collector’s belief: As the number of bidders increases, the effect of age on price also increases.
...
#### l. Test the overall utility of the model using the global F-test at 𝛼 = .05.
...
#### m. Test the hypothesis (at 𝛼 = .05) that the price–age slope increases as the number of bidders increases—that is, that age and number of bidders, x2, interact positively.
